...
static void WriteReceipt(const char* status, int shapes) {
    FILE* f = fopen("C:\\Users\\ASUS\\Documents\\L_edit\\agent-harness\\grayscale_test\\layout2_v2_receipt.txt", "w");
    if (f) {
        fprintf(f, "status=%s\nshapes=%d\n", status, shapes);
        fclose(f);
    }
...
static int GenerateGridDoseMatrix(LCell cell, LFile file) {
    ...
    // ???? (?10%?90%??8?)
    int doses[] = {10, 20, 30, 40, 50, 60, 70, 80};
    int num_doses = 8;
