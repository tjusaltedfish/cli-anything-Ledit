...
static void WriteReceipt(const char* status, int shapes) {
    FILE* f = fopen("C:\\Users\\ASUS\\Documents\\L_edit\\agent-harness\\grayscale_test\\layout1_v8_receipt.txt", "w");
    if (f) {
        fprintf(f, "status=%s\nshapes=%d\n", status, shapes);
        fclose(f);
    }
...
static int GenerateDoseCalibration(LCell cell, LFile file) {
    ...
    // ????????????????????????
    // 10/50=0.2mm, 10/60=0.167mm, 10/80=0.125mm, 10/100=0.1mm, 10/120=0.083mm
    double block_sizes[] = {10.0/50, 10.0/60, 10.0/80, 10.0/100, 10.0/120};
