...
static void WriteReceipt(const char* status, int shapes) {
    FILE* f = fopen("C:\\Users\\ASUS\\Documents\\L_edit\\agent-harness\\grayscale_test\\layout3_v4_receipt.txt", "w");
    if (f) {
        fprintf(f, "status=%s\nshapes=%d\n", status, shapes);
        fclose(f);
    }
...
static int GenerateAdjacencyComprehensive(LCell cell, LFile file) {
    ...
    // ??1: ??????
    {
        ...
        double region_width = region1_x2 - region1_x1;
        double region_height = region1_y2 - region1_y1;
        
        for (int i = 0; i < 5; i++) {
            double x = region1_x1 + i * (block_size * 2 + pair_gap + 0.2);
            double y = region1_y1 + (region_height - block_size) / 2;
            ...
            if (layer1) {
                LBox_New(cell, layer1, 
                    ...
                    LFile_DispUtoIntU(file, y + block_size));
                count++;
            }
    ...
    // ??2: ????
    {
        ...
        double region_width = region2_x2 - region2_x1;
        double region_height = region2_y2 - region2_y1;
        
        for (int i = 0; i < 5; i++) {
            double x = region2_x1 + i * (block_size * 2 + gaps[i] + 0.2);
            double y = region2_y1 + (region_height - block_size) / 2;
    ...
    // ??3: ????
    {
        ...
        double pair_gap = 0.1;
        double region_width = region3_x2 - region3_x1;
        double region_height = region3_y2 - region3_y1;
        ...
        if (layer30 && layer70) {
            ...
            double y_start = region3_y1 + (region_height - 2*block_size - pair_gap) / 2;
            ...
            // ????
            double y_offset = block_size + pair_gap;
            LBox_New(cell, layer30, 
                LFile_DispUtoIntU(file, x_start), 
    ...
    // ??4: ??????
    {
        double region_width = region4_x2 - region4_x1;
        double region_height = region4_y2 - region4_y1;
