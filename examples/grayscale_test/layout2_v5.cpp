...
static void WriteReceipt(const char* status, int shapes) {
    FILE* f = fopen("C:\\Users\\ASUS\\Documents\\L_edit\\agent-harness\\grayscale_test\\layout2_v5_receipt.txt", "w");
    if (f) {
        fprintf(f, "status=%s\nshapes=%d\n", status, shapes);
        fclose(f);
    }
...
static int GenerateGridDoseMatrix(LCell cell, LFile file) {
    ...
    // ??????
    double region_width = (usable - (num_cols - 1) * gap) / num_cols;
    double region_height = (usable - (num_rows - 1) * gap) / num_rows;
        ...
        for (int row = 0; row < num_rows; row++) {
            ...
            for (int r = 0; r < grids[col]; r++) {
                for (int c = 0; c < grids[col]; c++) {
                    ...
                    if (dist <= radius) {
                        // ??????????
                        if (px >= x1 && px + cell_size - 0.001 <= x2 &&
                            py >= y1 && py + cell_size - 0.001 <= y2) {
                            LBox_New(cell, layer, 
