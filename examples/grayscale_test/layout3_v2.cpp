...
static void WriteReceipt(const char* status, int shapes) {
    FILE* f = fopen("C:\\Users\\ASUS\\Documents\\L_edit\\agent-harness\\grayscale_test\\layout3_v2_receipt.txt", "w");
    if (f) {
        fprintf(f, "status=%s\nshapes=%d\n", status, shapes);
        fclose(f);
    }
...
static int GenerateAdjacencyComprehensive(LCell cell, LFile file) {
    ...
    // ????0.1mm?????????
    {
        ...
        double y_start = margin + usable * 0.55;
        double block_size = 1.5;
        double gap = 0.1;
        ...
        for (int i = 0; i < 5; i++) {
            double x = x_start + i * (block_size * 2 + gap + 0.3);
            ...
            if (layer2) {
                LBox_New(cell, layer2, 
                    LFile_DispUtoIntU(file, x + block_size + gap), 
                    LFile_DispUtoIntU(file, y_start), 
                    LFile_DispUtoIntU(file, x + 2*block_size + gap), 
                    LFile_DispUtoIntU(file, y_start + block_size));
                count++;
            }
        }
    ...
    {
        double x_start = margin;
        double y_start = margin + usable * 0.1;
        double block_size = 1.5;
        
        // ??: 0, 0.05, 0.1, 0.2, 0.5 mm
        double gaps[] = {0.0, 0.05, 0.1, 0.2, 0.5};
        
        for (int i = 0; i < 5; i++) {
            double x = x_start + i * (block_size * 2 + gaps[i] + 0.5);
    ...
    // ?????????????
    {
        double x_start = margin + usable * 0.55;
        double y_start = margin + usable * 0.55;
        ...
        if (layer30 && layer70) {
            ...
            LBox_New(cell, layer70, 
                LFile_DispUtoIntU(file, x_start + block_size + gap), 
                LFile_DispUtoIntU(file, y_start), 
                LFile_DispUtoIntU(file, x_start + 2*block_size + gap), 
                LFile_DispUtoIntU(file, y_start + block_size));
            count += 2;
            
            // ??
            double y_offset = block_size + gap + 0.5;
            LBox_New(cell, layer30, 
                LFile_DispUtoIntU(file, x_start), 
            ...
            LBox_New(cell, layer70, 
                LFile_DispUtoIntU(file, x_start), 
                LFile_DispUtoIntU(file, y_start + y_offset + block_size + gap), 
                LFile_DispUtoIntU(file, x_start + block_size), 
                LFile_DispUtoIntU(file, y_start + y_offset + 2*block_size + gap));
            count += 2;
            
            // ???
            double x_offset = block_size + gap + 0.5;
            LBox_New(cell, layer30, 
                ...
                LFile_DispUtoIntU(file, y_start + y_offset + block_size));
            LBox_New(cell, layer70, 
                LFile_DispUtoIntU(file, x_start + x_offset + block_size + gap), 
                LFile_DispUtoIntU(file, y_start + y_offset + block_size + gap), 
                LFile_DispUtoIntU(file, x_start + x_offset + 2*block_size + gap), 
                LFile_DispUtoIntU(file, y_start + y_offset + 2*block_size + gap));
            count += 2;
        }
    }
    ...
    // ??D: ?????? (???)
    {
        ...
        double step_width = usable * 0.4 / 21;
        double step_height = usable * 0.35;
